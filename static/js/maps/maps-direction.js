...
MedApp.maps.core = {
    ...
    initMap: function() {
      ...
      if (!window.google || !window.google.maps) {
        ...
        // 如果沒有加載，等待 API 加載完成
        const checkGoogleMapsLoaded = function() {
          if (window.google && window.google.maps) {
    ...
    setupSearchBox: function() {
      try {
        ...
        // 確保Google API和地圖已加載
        if (!window.google || !window.google.maps || !window.google.maps.places || !this.map) {
        ...
        MedApp.log("設置搜索框時出錯: " + error.message, 'error');
      }
    },
    ...
    selectLocation: function() {
      ...
      // 向聊天發送位置信息
      if (MedApp.chat.display && MedApp.chat.display.appendLocationMessage) {
